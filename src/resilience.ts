    ...
    for (const key in obj) {
      if (Object.prototype.hasOwnProperty.call(obj, key)) {
        if (truncateArraysRecursively(obj[key], depth + 1)) {
          truncated = true;
          // Keep a sibling row_count consistent with the truncated `rows`
          // length so callers see row_count === rows.length after truncation.
          // Gate on the `rows` key specifically — row_count describes `rows`,
          // not any other (possibly larger) array sibling like GA4's columns.
          if (key === "rows" && Array.isArray(obj[key]) && "row_count" in obj) {
