#!/usr/bin/env node
/**
 * Standalone OAuth2 refresh-token helper for mcp-ga4.
 *
 ...
 *
 * === When to use this (user-OAuth path) ===
 *   Use this if you want mcp-ga4 to act AS a Google user (e.g. your own GA4
 *   login). This is ga4's primary path. If instead you have a service-account
 *   JSON keyfile, you do NOT need this helper -- point
 *   GOOGLE_APPLICATION_CREDENTIALS at the keyfile and skip straight to running
 *   the server (see README, "Service account").
 *
 * === Prerequisites ===
 *   1. Google Cloud Console -> APIs & Services -> Credentials -> create an
 *      OAuth 2.0 Client ID of type "Desktop app". Enable the GA4 Data API and
 *      (if you use the admin tools) the GA4 Admin API.
 *   2. export GA4_CLIENT_ID=...  GA4_CLIENT_SECRET=...
 *   3. node get-refresh-token.cjs
 *   4. Approve in the browser (as the Google account that owns the GA4 data).
 *   5. Copy the printed GA4_REFRESH_TOKEN=... into your environment.
 *
 * The OAuth scope requested is read from this repo's config.json (oauth.scope),
 * falling back to the committed GA4 default -- the SAME source the MCP runtime
 * uses (src/oauthScope.ts), so helper and runtime never drift.
 */
...
// Committed default MUST match src/oauthScope.ts DEFAULT_GA4_SCOPE. ga4 ships a
// mutating tool (ga4_create_custom_dimension) that needs analytics.edit, so the
// default grants readonly + edit. A read-only user can override oauth.scope in
// config.json to analytics.readonly alone.
const DEFAULT_GA4_SCOPE =
...
// and src/authCli.ts). Google matches loopback redirect URIs on scheme + host +
...
/** Resolve oauth.scope from a parsed config object; falls back to the GA4 default. */
function resolveScopeFromConfig(config) {
  const scope =
    config && typeof config === "object" && config.oauth && typeof config.oauth.scope === "string"
      ? config.oauth.scope
      : "";
  return normalizeScope(scope) || DEFAULT_GA4_SCOPE;
}

/** Read oauth.scope from a config file on disk; falls back to the GA4 default. */
